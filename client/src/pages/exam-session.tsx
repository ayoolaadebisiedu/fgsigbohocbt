...
export default function ExamSessionPage() {
  ...
  const { data: session, isLoading: sessionLoading } = useQuery<ExamSession & { serverTime?: string }>({
  ...
  const submitExamMutation = useMutation<Result, unknown, void>({
    mutationFn: async () => {
      return submitExamSession(sessionId, answers);
    },
    onSuccess: (result) => {
    ...
    onError: (error) => {
      ...
      toast({
        ...
        description: "Failed to submit exam. Please try again or contact an administrator.",
        variant: "destructive",
      });
    }
  });
              ...
              <div className="grid grid-cols-8 gap-2 md:grid-cols-10">
                {questions.map((q, idx) => (
                  <Button
                    ...
                    className={`relative h-10 w-10 p-0 ${answers[q.id]
